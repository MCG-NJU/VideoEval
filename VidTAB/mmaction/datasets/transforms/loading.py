...
class DecordInit(BaseTransform):
    ...
    def transform(self, results):
        ...
        try:
            ...
            try:
                file_obj = io.BytesIO(client.Client(enable_mc=True).get(results['filename'], update_cache=True)) # 读出来有问题大概率是ceph缓存出错了
                container = decord.VideoReader(file_obj, num_threads=self.num_threads)
            except Exception as e2:
                print(f"\033[31m Encounter some errors when decording {results['filename']} !!!\033[0m")
                raise e2
        ...
        for i, frame_idx in enumerate(results['frame_inds']):
            ...
            if modality == 'RGB':
                ...
                try:
                    ...
                    try:
                        img_bytes = client.Client(enable_mc=True).get(filepath, update_cache=True) # 读出来有问题大概率是ceph缓存出错了
                        cur_frame = mmcv.imfrombytes(img_bytes, channel_order='rgb')
                    except Exception as e2:
                        print(f"\033[31m Encounter some errors when decording {filepath} !!!\033[0m")
                        raise e2
                imgs.append(cur_frame)
            elif modality == 'Flow':

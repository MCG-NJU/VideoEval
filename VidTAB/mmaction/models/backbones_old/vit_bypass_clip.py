...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, scale=1., num_adapters=1, num_frames=8, drop_path=0.):
        super().__init__()
        ...
        self.st_adapter_bypass = ST_Adapter(num_frames, d_model, skip_connect=False)
        self.num_frames = num_frames
    ...
    def forward(self, x: torch.Tensor):
        ...
        ## mlp with temporal bypass
        xn = self.ln_2(x)
        x = x + self.mlp(xn) + self.drop_path(self.scale * self.st_adapter_bypass(xn))
        return x


class Transformer(nn.Module):
    def __init__(self, num_frames, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, num_adapters=1, drop_path=0.1, scale=1.):
        super().__init__()
        self.width = width
        self.layers = layers
        dpr = [x.item() for x in torch.linspace(0, drop_path, self.layers)]
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask, scale, num_adapters, num_frames, dpr[i]) for i in range(layers)])
...
class ViT_Bypass_CLIP(nn.Module):
    ## ViT definition in CLIP image encoder
    def __init__(self, input_resolution: int, num_frames: int, patch_size: int, width: int, layers: int, heads: int, drop_path_rate, num_adapters=1, pretrained=None, scale=1.):
        super().__init__()
        ...
        self.transformer = Transformer(num_frames, width, layers, heads, num_adapters=num_adapters, drop_path=drop_path_rate, scale=scale)

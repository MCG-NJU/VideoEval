...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, scale=1., num_tadapter=1, num_frames=8, drop_path=0., use_time_attn=False, use_space_attn=True):
        super().__init__()
    ...
    def attention_rearrange(self, x: torch.Tensor):
        ...
        kv = x[1:, :, :].view(n, b, self.num_frames, d).permute(0, 2, 1, 3).contiguous().view(H, W, self.num_frames, b, d)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.use_time_attn:
            if self.num_tadapter == 2:
                xt = self.T_Adapter(self.attention_rearrange(
                    self.T_Adapter_in(self.ln_1(x))))
            else:
                xt = self.T_Adapter(self.attention_rearrange(self.ln_1(x)))
...
class Transformer(nn.Module):
    def __init__(self, num_frames, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, num_tadapter=1, scale=1., drop_path=0.1):
        ...
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask,                    # TODO 用了time shift就不用attn了
                                       scale, num_tadapter, num_frames, dpr[i], use_time_attn=True, use_space_attn=True) for i in range(layers)])
...
class ViT_Rearrange_CLIP(nn.Module):
    ...
    def __init__(self, input_resolution: int, num_frames: int, patch_size: int, width: int, layers: int, heads: int,
        ...
        self.transformer = Transformer(num_frames, width, layers, heads, num_tadapter=num_tadapter,
                                       scale=adapter_scale, drop_path=drop_path_rate)

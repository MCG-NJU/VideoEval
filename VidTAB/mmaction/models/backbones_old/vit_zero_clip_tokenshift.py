...
class TokenShift(nn.Module):
    def __init__(self, num_frames, n_div=8):
        super(TokenShift, self).__init__()
        self.num_frames = num_frames
        self.fold_div = n_div

        logger = MMLogger.get_current_instance()
        logger.info( f'Token shift, num_frames: {self.num_frames}, fold_div: {self.fold_div}')
    ...
    def forward(self, x):
        ...
        out[0, :, 1:, :fold] =  feat[0, :, :-1, :fold]  # shift left NOTE 假设第一个是cls token
...
class ResidualAttentionBlock_RepXViT(nn.Module):
    ...
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, scale=1., num_tadapter=1,
        ...
        self.shift_ops = TokenShift(num_frames=num_frames, n_div=self.xvit_cfg.get('n_div'))
...
class ViT_Zero_CLIP_token_shift(nn.Module):

...
class HalfGELUAdapter(nn.Module):
    ...
    def forward(self, x):
        # x is (HW+1, BT, D)
        xs = self.D_fc1(x)
        xs = torch.cat([xs[:, :, 0:self.half_dim], xs[:, :, self.half_dim:]], dim=-1)
        xs = self.dropout(xs)
        xs = self.D_fc2(xs)
...
class ResidualAttentionBlock_RepXViT(nn.Module):
    adapter_map = {'rep': RepAdapter, 'gelu': GELUAdapter, 'half_gelu': HalfGELUAdapter}
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, scale=1., num_tadapter=1,
        ...
        self.attn = XShiftMultiheadAttention_ablation(embed_dim=d_model, num_heads=n_head, 
                num_frames=num_frames, shift_div=self.xvit_cfg.get('shift_div', 12), shift_pattern=self.xvit_cfg.get('shift_pattern', 'kv'),
                 ops_type=self.xvit_cfg.get('ops_type'), shift_stride=self.xvit_cfg.get('shift_stride', 1),

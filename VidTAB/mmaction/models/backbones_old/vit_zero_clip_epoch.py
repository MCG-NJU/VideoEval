...
class RepAdapter(nn.Module):
    ...
    def forward(self, x, act_alpha):
        # x is (HW+1, BT, D)
        xs = self.D_fc1(x)
        xs = self.D_fc2(xs)
        xs = act_alpha * xs + (1 - act_alpha) * self.act(xs)
...
class ResidualAttentionBlock_RepXViT(nn.Module):
    ...
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, scale=1., num_tadapter=1,
                 num_frames=8, adapter_type='rep', mlp_ratio=0.25, xvit_cfg=dict()):
        super().__init__()
        ...
        logger.info(f'num_tadapter:{num_tadapter}, mlp_ratio:{mlp_ratio} scale:{scale}')

        if adapter_type == 'rep':
            self.MLP_Adapter = RepAdapter(d_model, scale=scale, mlp_ratio=mlp_ratio)
            if self.num_tadapter == 2:
                self.MLP_Adapter_out = RepAdapter(d_model, scale=scale, mlp_ratio=mlp_ratio)

            self.S_Adapter = RepAdapter(d_model, scale=scale, mlp_ratio=mlp_ratio)
            self.T_Adapter_in = RepAdapter(d_model, scale=scale, mlp_ratio=mlp_ratio)
        else:
    ...
    def forward(self, x: torch.Tensor, act_alpha: int):
        # x shape [HW+1, BT, D]
        # spatial adaptation with shift
        x = x + self.S_Adapter(self.attention(self.T_Adapter_in(self.ln_1(x), act_alpha)), act_alpha)
        # joint adaptation
        if self.num_tadapter == 2:
            x = x + self.MLP_Adapter_out(self.mlp(self.MLP_Adapter(self.ln_2(x), act_alpha)), act_alpha)
        else:
            x = x + self.mlp(self.MLP_Adapter(self.ln_2(x), act_alpha))
        return x

    
class Transformer(nn.Module):
    def __init__(self, num_frames, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, num_tadapter=1, scale=1., adapter_type='rep', mlp_ratio=0.25, xvit_cfg=dict()):
        super().__init__()
        self.width = width
        self.layers = layers
        # dpr = [x.item() for x in torch.linspace(0, drop_path, self.layers)]
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock_RepXViT(width, heads, attn_mask, scale, num_tadapter, num_frames, adapter_type=adapter_type, mlp_ratio=mlp_ratio, xvit_cfg=xvit_cfg) for i in range(layers)])

    def forward(self, x: torch.Tensor, act_alpha: int):
        for i in range(len(self.resblocks)):
            x = self.resblocks[i](x, act_alpha)
        return x


@MODELS.register_module()
class ViT_Zero_CLIP_epoch(nn.Module):
    ## ViT definition in CLIP image encoder
    def __init__(self, input_resolution: int, num_frames: int, patch_size: int, width: int, layers: int, heads: int, num_tadapter=1, adapter_scale=0.5, pretrained=None, adapter_type='rep', mlp_ratio=0.25, xvit_cfg=dict(), final_ta_cfg=dict()):
        super().__init__()
        ...
        self.transformer = Transformer(num_frames, width, layers, heads, num_tadapter=num_tadapter, scale=adapter_scale, adapter_type=adapter_type, mlp_ratio=mlp_ratio, xvit_cfg=xvit_cfg)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.transformer(x, self.current_epoch / self.max_epochs) # NOTE 通过hook传过来

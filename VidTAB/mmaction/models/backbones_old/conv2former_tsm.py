...
class NL3DWrapper(nn.Module):
    """3D Non-local wrapper for Conv2Former.

    Wrap Conv2Former layers with 3D NonLocal modules.
...
class Conv2FormerTSM(Conv2Former):
    """Conv2Former backbone for TSM.
    ...
    Args:
        ...
        shift_place (str): Places in Conv2Former layers for shift, which is chosen
            from ['block', 'blockres'].
            If set to 'block', it will apply temporal shift to all child blocks
            in each Conv2Former layer.
            If set to 'blockres', it will apply temporal shift to each `conv1`
            layer of all child blocks in each Conv2Former layer.
            Default: 'blockres'.
    ...
    def make_temporal_shift(self):
        ...
        if self.shift_place == 'block':
            ...
            for i in range(self.num_stages):
                make_block_temporal(self.stages[i], num_segment_list[i])
    ...
    def make_temporal_pool(self):
        ...
        class TemporalPool(nn.Module):
            ...
            Wrap layer2 in Conv2Former with a 3D max pooling layer.
        ...
        self.stages[1] = TemporalPool(self.stages[1], self.num_segments)

    def make_non_local(self):
        # This part is for Conv2Former
        for i in range(self.num_stages):
            ...
            for idx, non_local in enumerate(non_local_stage):
                if non_local:
                    self.stages[i][idx] = NL3DWrapper(self.stages[i][idx],
                                                 self.num_segments,
                                                 self.non_local_cfg)

...
class RepAdapter(nn.Module):
    def __init__(self, D_features, mlp_ratio=0.25, skip_connect=True, scale=1., dropout_rate=0.1, groups=2):
        super().__init__()
        self.skip_connect = skip_connect
        D_hidden_features = int(D_features * mlp_ratio)
        self.D_fc1 = nn.Conv1d(D_features, D_hidden_features, 1, groups=1,bias=True)
        self.dropout = nn.Dropout(dropout_rate)  #, inplace=True)
        self.D_fc2 = nn.Conv1d(D_hidden_features, D_features, 1, groups=groups, bias=True)
        self.scale=scale
    ...
    def forward(self, x):
        # x is (HW+1, BT, D)
        xs = self.D_fc1(x.transpose(1, 2))
        xs = self.dropout(xs)
        xs = self.D_fc2(xs).transpose(1, 2)
...
class ResidualAttentionBlock_RepXViT(nn.Module):
    ...
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, scale=1., num_tadapter=1,
        ...
        if adapter_type == 'rep':
            self.MLP_Adapter = RepAdapter(d_model, scale=scale, mlp_ratio=mlp_ratio, dropout_rate=dropout_rate, groups=self.xvit_cfg.get('groups', 2))
            if self.num_tadapter == 2:
                self.MLP_Adapter_out = RepAdapter(d_model, scale=scale, mlp_ratio=mlp_ratio, dropout_rate=dropout_rate, groups=self.xvit_cfg.get('groups', 2))

            self.S_Adapter = RepAdapter(d_model, scale=scale, mlp_ratio=mlp_ratio, dropout_rate=dropout_rate, groups=self.xvit_cfg.get('groups', 2))
            self.T_Adapter_in = RepAdapter(d_model, scale=scale, mlp_ratio=mlp_ratio, dropout_rate=dropout_rate, groups=self.xvit_cfg.get('groups', 2))
        else:
...
class Transformer(nn.Module):
    def __init__(self, num_frames, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, num_tadapter=1, scale=1., dropout_rate=0.1, adapter_type='rep', mlp_ratio=0.25, xvit_cfg=dict()):
        super().__init__()
        self.width = width
        self.layers = layers
        # dpr = [x.item() for x in torch.linspace(0, drop_path, self.layers)]
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock_RepXViT(width, heads, attn_mask, scale, num_tadapter, num_frames, dropout_rate=dropout_rate, adapter_type=adapter_type, mlp_ratio=mlp_ratio, xvit_cfg=xvit_cfg) for i in range(layers)])
...
class ViT_Zero_CLIP_rep(nn.Module):
    ## ViT definition in CLIP image encoder
    def __init__(self, input_resolution: int, num_frames: int, patch_size: int, width: int, layers: int, heads: int,
     dropout_rate=0.1, num_tadapter=1, adapter_scale=0.5, pretrained=None, adapter_type='rep', mlp_ratio=0.25, xvit_cfg=dict(), final_ta_cfg=dict()):
        super().__init__()
        ...
        self.transformer = Transformer(num_frames, width, layers, heads, num_tadapter=num_tadapter, scale=adapter_scale, dropout_rate=dropout_rate, adapter_type=adapter_type, mlp_ratio=mlp_ratio, xvit_cfg=xvit_cfg)

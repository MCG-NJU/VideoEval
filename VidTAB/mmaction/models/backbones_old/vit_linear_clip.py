...
class LinearAdapter(nn.Module):
    def __init__(self, D_features, mlp_ratio=0.25, act_layer=nn.GELU, skip_connect=True, mid_act=False, last_act=False):
        super().__init__()
        self.skip_connect = skip_connect
        self.mid_act = mid_act
        self.last_act = last_act
        D_hidden_features = int(D_features * mlp_ratio)
        if mid_act or last_act:
            self.act = act_layer()
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, scale=1., num_tadapter=1,
                  num_frames=8, drop_path=0., motion_cfg=dict(), use_frame_attn=True, use_space_attn=True, use_time_shift=False, adapter_cfg=dict(mid_act=False, last_act=False)):
        super().__init__()
        ...
        if self.use_time_attn or self.use_frame_attn:
            self.T_Adapter = LinearAdapter(d_model, skip_connect=False, **adapter_cfg)
            if num_tadapter == 2:
                self.T_Adapter_in = LinearAdapter(d_model, **adapter_cfg)
        self.num_frames = num_frames
        ...
        if use_space_attn:
            self.S_Adapter = LinearAdapter(d_model, **adapter_cfg)

        self.MLP_Adapter = LinearAdapter(d_model, skip_connect=False, **adapter_cfg)
...
class Transformer(nn.Module):
    def __init__(self, num_frames, width: int, layers: int, heads: int,
                  attn_mask: torch.Tensor = None, num_tadapter=1, scale=1., drop_path=0.1,
                    motion_cfg=dict(), use_time_shift=False, allow_time_space=True, adapter_cfg=dict(mid_act=False, last_act=False)):
        super().__init__()
        self.width = width
        self.layers = layers
        dpr = [x.item() for x in torch.linspace(0, drop_path, self.layers)]
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask,                    
                                       scale, num_tadapter, num_frames, dpr[i], motion_cfg, use_frame_attn= i % 2 != 0, use_space_attn=True if (allow_time_space or i % 2 == 0) else False, use_time_shift=use_time_shift, adapter_cfg=adapter_cfg) for i in range(layers)])
...
class ViT_Linear_CLIP(nn.Module):
    # ViT definition in CLIP image encoder
    def __init__(self, input_resolution: int, num_frames: int, patch_size: int, width: int, layers: int, heads: int,
                 drop_path_rate, num_tadapter=1, adapter_scale=0.5, pretrained=None, motion_cfg=dict(),
                 use_time_shift=False, allow_time_space=True, adapter_cfg=dict(mid_act=False, last_act=False)):
        super().__init__()
        ...
        self.transformer = Transformer(num_frames, width, layers, heads, num_tadapter=num_tadapter,
                                       scale=adapter_scale, drop_path=drop_path_rate, motion_cfg=motion_cfg,
                                         use_time_shift=use_time_shift, allow_time_space=allow_time_space, adapter_cfg=adapter_cfg)

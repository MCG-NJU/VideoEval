...
from ..common import XShiftMultiheadAttention_shift
...
class ResidualAttentionBlock_RepXViT(nn.Module):
    adapter_map = {'rep': RepAdapter} # , 'gelu': GELUAdapter}
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, scale=1., num_tadapter=1,
                 num_frames=8, dropout_rate=0.1, adapter_type='rep', mlp_ratio=0.25, xvit_cfg=dict(), attn_adapter_type='share'):
        super().__init__()
        self.num_tadapter = num_tadapter
        self.xvit_cfg = xvit_cfg
        self.attn = XShiftMultiheadAttention_shift(embed_dim=d_model, num_heads=n_head, 
                num_frames=num_frames,shift_pattern=self.xvit_cfg.get('shift_pattern', 'kv'),
                 divide_head=self.xvit_cfg.get('divide_head', False),
                  shift1_div=self.xvit_cfg.get('shift1_div', 12), 
                  shift2_div=self.xvit_cfg.get('shift2_div', -1), 
                  shift3_div=self.xvit_cfg.get('shift3_div', -1), 
                  shift3_right=self.xvit_cfg.get('shift3_right', False),
                  shift4_div=self.xvit_cfg.get('shift4_div', -1), 
                  shift4_right=self.xvit_cfg.get('shift4_right', False),
                 )
        ...
        logger.info(f'num_tadapter:{num_tadapter}, mlp_ratio:{mlp_ratio} scale:{scale} dropout_rate:{dropout_rate} xvit_cfg: {xvit_cfg}')
...
class Transformer(nn.Module):
    def __init__(self, num_frames, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, num_tadapter=1, scale=1., dropout_rate=0.1, adapter_type='rep', mlp_ratio=0.25, xvit_cfg=dict(), attn_adapter_type='share'):
        super().__init__()
        self.width = width
        self.layers = layers
        # dpr = [x.item() for x in torch.linspace(0, drop_path, self.layers)]
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock_RepXViT(width, heads, attn_mask, scale, num_tadapter, num_frames, dropout_rate=dropout_rate, adapter_type=adapter_type, mlp_ratio=mlp_ratio, xvit_cfg=xvit_cfg, attn_adapter_type=attn_adapter_type) for i in range(layers)])
...
class ViT_Zero_CLIP_shift(nn.Module):
    ## ViT definition in CLIP image encoder
    def __init__(self, input_resolution: int, num_frames: int, patch_size: int, width: int, layers: int, heads: int,
     dropout_rate=0.1, num_tadapter=1, adapter_scale=0.5, pretrained=None, adapter_type='rep', mlp_ratio=0.25, xvit_cfg=dict(), final_ta_cfg=dict(), attn_adapter_type='share'):
        super().__init__()
        ...
        self.transformer = Transformer(num_frames, width, layers, heads, num_tadapter=num_tadapter, scale=adapter_scale, dropout_rate=dropout_rate, adapter_type=adapter_type, mlp_ratio=mlp_ratio, xvit_cfg=xvit_cfg, attn_adapter_type=attn_adapter_type)

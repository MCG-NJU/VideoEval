...
class HeadReallocation(nn.Module):
    ...
    def shift_pos(self, x_pos, frame_len):
        # x_pos is (Frames, nH, Wd*Wh*Ww, Wd*Wh*Ww)
        if self.num_theads < 1: # 小于1不shift
            return x_pos
        
        assert frame_len == x_pos.shape[0] == self.num_frames
        out = x_pos.clone()
    ...
    def forward(self, x, batch_size, frame_len):
        ...
        if self.num_theads < 0: 
            ...
            if self.num_theads < -1: # channel shift
                # print('channel shift', self.num_theads)
                ...
                # print('head shift', self.num_theads)
                feat = feat.permute(0, 2, 1, 3).reshape(batch_size, frame_len, -1, N, num_heads * c)
                out = feat.clone()
                fold = num_heads * c // 8
                out[:, 1:, :, :, :fold] = feat[:, :-1, :, :, :fold]  # shift left
                out[:, :-1, :, :, fold:2*fold] = feat[:, 1:, :, :, fold:2*fold]  # shift right
                out = out.view(batch_size, frame_len, -1, N, num_heads, c).permute(0, 1, 2, 4, 3, 5)
        else:
            ...
            if self.num_theads < 1:
                # print('test', self.num_theads)
                out[:, 1:, :, 0, :, :c//2] = feat[:, :-1, :, 0, :, :c//2]  # shift left
                out[:, :-1, :, 0, :, c//2:] = feat[:, 1:, :, 0, :, c//2:]  # shift right
...
class WindowAttention2D(BaseModule):
    ...
    def forward(self,
        ...
        k = self.shift_ops(k, batch_size=batch_size, frame_len=frame_len)
        v = self.shift_ops(v, batch_size=batch_size, frame_len=frame_len)
        ...
        if self.zero_cfg.get('shift_pos', True):
            # print('shift_ops')
            relative_position_bias = self.shift_ops.shift_pos(relative_position_bias, frame_len=frame_len)
...
class SwinTransformer2D_Zero_fix(BaseModule):
    ...
    def train(self, mode: bool = True) -> None:
        ...
        super(SwinTransformer2D_Zero_fix, self).train(mode)

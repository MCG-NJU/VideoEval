...
class TokenShift(nn.Module):
    def __init__(self, num_frames, n_head, n_div=8):
        ...
        self.n_head = n_head
        print(
            f'=> Using token shift, num_frames: {self.num_frames}, n_head: {self.n_head}, fold_div: {self.fold_div}')
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, scale=1., num_tadapter=1, num_frames=8, drop_path=0., use_time_shift=True, shift_type='channel_shift'):
        super().__init__()
        ...
        if use_time_shift:
            ...
            if shift_type == 'channel_shift':
                self.time_shift = TemporalShift(
                    num_frames=num_frames, n_head=n_head)
            elif shift_type == 'token_shift':
                self.time_shift = TokenShift(
                    num_frames=num_frames, n_head=n_head)
            else:
...
class Transformer(nn.Module):
    def __init__(self, num_frames, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, num_tadapter=1, scale=1., drop_path=0.1, use_time_shift=True, shift_type='channel_shift'):
        super().__init__()
        self.width = width
        self.layers = layers
        dpr = [x.item() for x in torch.linspace(0, drop_path, self.layers)]
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask,                    # TODO 用了time shift就不用attn了
                                       scale, num_tadapter, num_frames, dpr[i], use_time_shift=use_time_shift, shift_type=shift_type) for i in range(layers)])
...
class ViT_Shift_CLIP(nn.Module):
    # ViT definition in CLIP image encoder
    def __init__(self, input_resolution: int, num_frames: int, patch_size: int, width: int, layers: int, heads: int,
                 drop_path_rate, num_tadapter=1, adapter_scale=0.5, pretrained=None, use_time_shift=True, shift_type='channel_shift'):
        super().__init__()
        ...
        self.transformer = Transformer(num_frames, width, layers, heads, num_tadapter=num_tadapter,
                                       scale=adapter_scale, drop_path=drop_path_rate, use_time_shift=use_time_shift, shift_type=shift_type)

...
class ResidualAttentionBlock_RepXViT(nn.Module):
    ...
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, scale=1., num_tadapter=1,
        ...
        if adapter_type == 'rep':
            ...
            self.T_Adapter_in = RepAdapter(d_model, scale=scale, mlp_ratio=xvit_cfg.get('T_Adapter_ratio', 0.25), dropout_rate=dropout_rate)
        else:
...
class Transformer(nn.Module):
    def __init__(self, num_frames, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, num_tadapter=1, scale=1., dropout_rate=0.1, adapter_type='rep', mlp_ratio=0.25, xvit_cfg=dict()):
        ...
        if xvit_cfg.get('use_descend', False):
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock_RepXViT(width, heads, attn_mask, scale, num_tadapter, num_frames, dropout_rate=dropout_rate, adapter_type=adapter_type, mlp_ratio=0.5*(mlp_ratio+mlp_ratio*(layers-i)/layers), xvit_cfg=xvit_cfg) for i in range(layers)])
        else:
...
class ViT_Zero_CLIP_rank(nn.Module):

...
from ..common import AttentionFrameSampler
...
@MODELS.register_module()
class TimeSformerWithAdaptiveSampler(TimeSformer):
    def __init__(self, num_frames, img_size, patch_size, pretrained=None, freeze_model=False, 
        embed_dims=768, num_heads=12, num_transformer_layers=12,
        in_channels=3, dropout_ratio=0, transformer_layers=None, attention_type='divided_space_time', norm_cfg=dict(type='LN', eps=0.000001), **kwargs):
        super().__init__(num_frames, img_size, patch_size, pretrained, embed_dims, num_heads,
                         num_transformer_layers, in_channels, dropout_ratio, transformer_layers, attention_type, norm_cfg, **kwargs)
        self.freeze_model = freeze_model
        self.sampler = AttentionFrameSampler(8)
        
    def forward(self, x):
        """Defines the computation performed at every call."""
        # input x N C T H W
        x = self.sampler(x.permute(0, 2, 1, 3, 4).contiguous())
        return super().forward(x.permute(0, 2, 1, 3, 4).contiguous())

    def _freeze_model(self) -> None:
        """Freeze all parameters."""
        for name, m in self.named_children():
            if 'sampler' not in name: # 冻结sampler以外的参数 noqa lxh
                m.eval()

...
from .swin_refiner import WindowAttention2D, TimeRefiner
# from ..utils import print_cuda_memory


class InterFrameWindowAttention2D(BaseModule): # 不直接继承而是改是避免耦合太严重
    ...
    def __init__(self,
        ...
        if self.ifa_cfg.get('use_motion', False):
            self.motion_dims = embed_dims
            # int(embed_dims * ifa_cfg.get('embed_ratio', 1)) # TODO embed后面可以视情况修改，比如改成 embed_dims / head，不一定要用多头
            self.loc_embed = nn.Linear(2, self.motion_dims, bias=qkv_bias)
            self.motion_proj = nn.Linear(self.motion_dims, self.motion_dims)
    ...
    def forward(self,
                ...
                batch_size=8, frame_len=8,
                loc_map=None) -> torch.Tensor:
        """Forward function.
        ...
        k_shape = k.shape
        old_k = k.view(batch_size, frame_len, self.num_spatial_windows,
                   self.num_heads, N,  C // self.num_heads)
        old_v = v.view(batch_size, frame_len, self.num_spatial_windows,
                   self.num_heads, N,  C // self.num_heads)

        k = old_k.clone()
        v = old_v.clone()
        # 为了复用qkv的预训练权重只能k，v都搞一遍
        k[:, :-1, :, :, :], v[:, :-1, :, :, :] = old_k[:, 1:, :, :,
                                                   :], old_v[:, 1:, :, :, :]  # shift一个时间位
        k, v = k.view(k_shape), v.view(k_shape)

        q = q * self.scale
        attn_origin = q @ k.transpose(-2, -1)
        # 考虑到motion是通过相似度而非位置来得到，这里先和带相对位置编码的attn分离 TODO

        if self.ifa_cfg.get('use_motion', False):
            # print('make motion')
            attn_motion = attn_origin.view(-1, self.num_heads, N, N)
            if mask is not None:
                nW = mask.shape[0]
                attn_motion = attn_motion.view(B_ // nW, nW, self.num_heads, N,
                                               N) + mask.unsqueeze(1).unsqueeze(0)
                attn_motion = attn_motion.view(-1, self.num_heads, N, N)
                attn_motion = self.softmax(attn_motion)
            else:
                attn_motion = self.softmax(attn_motion)
            ...
            x_motion = self.motion_proj(loc_next_embed - loc_embed) # TODO 单向的话最后一位motion是没用的，或者考虑改成双向的
        ...
        attn = attn_origin.view(batch_size, frame_len, -1, self.num_heads, N, N).permute(
            0, 2, 1, 3, 4, 5) + relative_position_bias.unsqueeze(0).unsqueeze(1)  # B_, nH, N, N TODO tsm的位置编码考虑要shift
        ...
        if self.refine_cfg.get('type') is not None:
            if self.refine_cfg.get('window4channel', False):
                ...
                attn = attn + self.attn_refiner(attn)
...
class SwinTransformerBlock3D(BaseModule):
    ...
    def __init__(self,
        ...
        if use_ifa:
            # _attn_cfg['shift'] = False  # TODO InterFrame先不shift了 shift 他妈的
            _attn_cfg['ifa_cfg'] = ifa_cfg
            if self.ifa_cfg.get('fuse_type', '') == 'concat':
                self.fuse_fc = nn.Linear(embed_dims*2, embed_dims)

            self.attn = InterFrameWindowAttention2D(**_attn_cfg)
        else:
    ...
    def forward_part1(self, x: torch.Tensor,
        ...
        if self.use_ifa and self.ifa_cfg.get('use_motion', False):
            ...
            # W-MSA/SW-MSA
            attn_windows, x_motion = self.attn(
...
class BasicLayer(BaseModule):
    ...
    def __init__(self,
        ...
        for i in range(depth):
            ...
            if (i == 0) or (i % 3 == 0):  # TODO 随便设一个规则试试
                _block_cfg['use_ifa'] = True
                _block_cfg['ifa_cfg'] = ifa_cfg
...
class SwinTransformer2D_IFA(BaseModule):
    ...
    def train(self, mode: bool = True) -> None:
        ...
        super(SwinTransformer2D_IFA, self).train(mode)
        self._freeze_stages()

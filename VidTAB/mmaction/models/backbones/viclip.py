...
class ViCLIP(nn.Module):
    def __init__(
        ...
        temp_embed=True, pretrained=None, pretrained_type='ViCLIP', 
        dpe_cfg=None, adaptation_type='frozen_tuning'
    ):
        super().__init__()
    ...
    def init_weights(self):
        ...
        logger.info(f"pretrained: {self.pretrained}, pretrained type: {self.pretrained_type}, adaptation type: {self.adaptation_type}")
        if self.pretrained is not None:
            ...
            load_state_dict_lyz(self, new_state_dict, input_resolution=self.input_resolution, patch_size=self.patch_size, center=True, logger=logger, strict=(self.pretrained_type != 'CLIP' and self.dpe_cfg is None and self.adaptation_type in ['full_finetuning', 'linear_probe']))

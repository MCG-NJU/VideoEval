...
from .internvideo2_src.pos_embed import get_3d_sincos_pos_embed, get_2d_sincos_pos_embed, get_1d_sincos_pos_embed, interpolate_pos_embed_umtv2
from .internvideo2_src.flash_attention_class import FlashAttention
    ...
    def forward(self, x, residual=None):
        ...
        def _inner_forward(x, residual=None):
            if self.use_fused_rmsnorm:
                ...
                x = self.drop_path1(self.ls1(self.attn_adapter_out(self.attn(self.attn_adapter_in(x)))))
                x, residual = self.norm2(x, residual)
                x = self.drop_path2(self.ls2(self.mlp_adapter_out(self.mlp(self.mlp_adapter_in(x)))))
                return x, residual
            else:
                assert residual is None
                x = x + self.drop_path1(self.ls1(self.attn_adapter_out(self.attn(self.attn_adapter_in(self.norm1(x))))))
                x = x + self.drop_path2(self.ls2(self.mlp_adapter_out(self.mlp(self.mlp_adapter_in(self.norm2(x))))))
                return x
...
class InternVideo2(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.adaptation_type == 'frozen_tuning':
            x = self.attentive_pooler(x).squeeze(dim=1)
        else:
            x = x.mean(dim=1)
    ...
    def init_weights(self):
        logger = MMLogger.get_current_instance()
        logger.info(f"pretrained: {self.pretrained}, adaptation type: {self.adaptation_type}")
        if self.pretrained is not None:
            state_dict = torch.load(self.pretrained, map_location='cpu')
            if self.num_frames != 8 or self.orig_t_size != 8:
                interpolate_pos_embed_umtv2(state_dict, self, orig_t_size=self.orig_t_size) # NOTE 单模态预训练为8
...
'''
def InternVideo2_1B_patch14_224(config):
    ...
    if config.vision_encoder.pretrained is not None:
        ...
        interpolate_pos_embed_umtv2(state_dict, model, orig_t_size=8) # NOTE 单模态预训练为8
...
def InternVideo2_6B_patch14_224(config):
    model = PretrainVisionTransformer_clean(
        in_chans=3, img_size=224, patch_size=14,

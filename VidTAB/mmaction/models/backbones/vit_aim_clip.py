...
class Adapter(nn.Module):
    def __init__(self, D_features, mlp_ratio=0.125, act_layer=nn.GELU, skip_connect=True):
        super().__init__()
...
class ViT_AIM_CLIP(nn.Module):
    ## ViT definition in CLIP image encoder
    def __init__(self, input_resolution: int, num_frames: int, patch_size: int, width: int, layers: int, heads: int, drop_path_rate, num_tadapter=1, adapter_scale=0.5, pretrained=None, mlp_ratio=0.125):
        super().__init__()
        self.input_resolution = input_resolution
        self.pretrained = pretrained
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False)

        assert mlp_ratio == 0.125

...
class ViT_ST_Adapter_CLIP(nn.Module):
    ...
    def __init__(self, input_resolution: int, num_frames: int, patch_size: int, width: int, layers: int, heads: int, drop_path_rate, num_tadapter=1, pretrained=None, adapter_type='st', use_flash=False):
        super().__init__()
        ...
        self.transformer = Transformer(num_frames, width, layers, heads, num_adapters=num_tadapter, drop_path=drop_path_rate, adapter_type=adapter_type, use_flash=use_flash)
